...
reg_theta_sum = sum(sum(reg_Theta1.^2))...
							 + sum(sum(reg_Theta2.^2));

reg_cost = (lambda/(2*m))*reg_theta_sum;
...
for t = 1:m
  %%FF
	...
	sample_y = yMat(t,:);
	d_3 = a3-sample_y; %1x10

	d_2 = (d_3*Theta2) .* sigmoidGradient([1 z2]); % 1 x 26
	d_2 = d_2(2:end); 
